...
async def promote(promt):
    ...
    if not rank:
        rank = "Raganork-mine"  # Just in case.
    if user:
        pass
    else:
        return
...
async def unmoot(unmot):
    ...
    if unmute(unmot.chat_id, user.id) is False:
        ...
        try:
            ...
            await unmot.edit("```Unmuted Successfully             Now this user can talk freely ```")
        except UserIdInvalidError:
...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "`No deleted accounts found, this place is Clean like hell`"
...
async def kick(usr):
    ...
    if not user:
        await usr.edit("`Not able to fetch the user.`")
        return
